...
1. **`handleDeepLink` e apenas um stub**: A funcao em `usePushNotifications.ts` apenas faz `console.log('[DeepLink] data:', data)` sem realizar navegacao real. O criterio exige mapeamento `aquabook://aquarium/{id}` → AquariumDetail, `aquabook://chat/{id}` → ChatDetail, etc. A navegacao global precisa ser implementada ou pelo menos o roteamento via `navigation.navigate()` com referencia ao navigator.

2. **`useLogoutCleanup.ts` nao e um hook**: O arquivo exporta `performLogout` como funcao async direta, nao como um hook React (`useLogoutCleanup`). O criterio especifica explicitamente um hook `useLogoutCleanup.ts`. O nome e a assinatura devem ser corrigidos para seguir a convencao de hooks.
