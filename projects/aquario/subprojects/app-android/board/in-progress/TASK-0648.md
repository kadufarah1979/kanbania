...
- [ ] Fontes carregadas via `expo-google-fonts`: Inter (400, 500, 600, 700) + JetBrains Mono (400, 700)
- [ ] Hook `useFonts` no `_layout.tsx` com splash screen mantida ate fontes carregadas
...
1. **mobile/package.json**: pacotes `@expo-google-fonts/inter` e `@expo-google-fonts/jetbrains-mono` ausentes. Apenas `expo-font ~13.0.2` esta instalado, sem as familias Inter e JetBrains Mono. Adicionar ambos os pacotes e instalar.

2. **mobile/src/app/_layout.tsx**: sem implementacao de `useFonts` (de `expo-font` ou `@expo-google-fonts/*`) e sem `SplashScreen.preventAutoHideAsync()`. O criterio exige que a splash screen seja mantida ate as fontes carregarem. Implementar conforme criterio.

3. **mobile/src/shared/components/EmptyState.tsx:3**: import `Button` usa caminho relativo `'./Button'`. Deve ser `'@/shared/components/Button'` (import absoluto via `@/`).

4. **mobile/src/shared/components/PlanGate.tsx:7**: import `Button` usa caminho relativo `'./Button'`. Deve ser `'@/shared/components/Button'` (import absoluto via `@/`).
