...
## Correcoes Aplicadas (resubmit)

1. **`handleDeepLink` com navegacao real**: Criado `mobile/src/shared/navigation/navigationRef.ts` com `createNavigationContainerRef`. `RootNavigator.tsx` recebe `ref={navigationRef}`. `handleDeepLink` navega para `AquariumDetail` ou `ChatDetail` via `navigationRef.navigate()` apos verificar `navigationRef.isReady()`.

2. **`useLogoutCleanup` refatorado para hook React**: Usa `useCallback`, `useQueryClient`, `useAuthStore` e `useNotificationStore` como hooks React. Retorna `{ performLogout }`.
