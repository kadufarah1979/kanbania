...
- [ ] `src/shared/constants/tokens.ts` criado com todos os tokens de `mobile/ux/design-system/tokens.ts` (colors, spacing, textStyles, shadows, duration)
- [ ] NativeWind 4 configurado: `tailwind.config.js` + `global.css` + `nativewind-env.d.ts`
...
- [ ] Componentes implementados em `src/shared/components/`:
...
- [ ] `src/shared/components/index.ts` com re-exports de todos os componentes
- [ ] Storybook-like: tela `DesignSystemScreen` (dev only) listando todos os componentes
...
- Button deve ter `activeOpacity` via `useAnimatedStyle` (scale 0.97 no press)
- Cores de status: ok=#34D399, warn=#FBBF24, crit=#F87171 — nao alterar
