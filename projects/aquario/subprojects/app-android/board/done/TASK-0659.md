...
acted_by:
  ...
  - agent: "claude-code"
    action: implemented
    date: "2026-02-26T16:35:00-03:00"
  - agent: "claude-code"
...
- [x] Model `FcmToken`: `id`, `user_id` (FK), `token` (string, unique), `platform` (android/ios), `created_at`
- [x] Migration Alembic: tabela `fcm_tokens`
- [x] `POST /api/v1/notifications/fcm-token` (autenticado):
...
- [x] `DELETE /api/v1/notifications/fcm-token` (autenticado):
  - Body: `{ token: str }`
  - Remove token do banco (logout ou desativacao de push)
- [x] `notification_service.py`: adicionar `send_fcm_notification(user_id, title, body, data)`:
...
- [x] `alert_service.py`: `dispatch_alert_notification` chama `send_fcm_notification` alem do Web Push existente (channel_id="aquabook_alerts")
- [x] `FIREBASE_CREDENTIALS_JSON` no `.env.example` e `docker-compose.yml`
...
- Branch: `task/TASK-0659`
- Commit: `5280eac feat(notifications): modelo FcmToken + endpoints fcm-token (TASK-0659)`
- firebase-admin inicializa lazily via FIREBASE_CREDENTIALS_JSON; se nao configurado, loga warning e retorna silenciosamente
- chat_service.py nao tinha notificacoes push — FCM integrado apenas via dispatch_alert_notification
