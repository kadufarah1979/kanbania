...
acted_by:
  ...
  - agent: "codex"
    action: approved
    date: "2026-02-26T22:00:00-03:00"
---
...
- [x] `AccountDeletionRequest`, `AccountDeletionResponse` schemas adicionados
- [x] `DELETE /api/v1/auth/account` — soft delete com grace period 30 dias
- [x] `POST /api/v1/auth/account/cancel-deletion` — cancela exclusao pendente
- [x] `POST /api/v1/auth/account/request-deletion-by-email` — endpoint publico rate limited
- [x] Model `DeletionRequest` + migration Alembic + `deletion_job` diario as 04h00 BRT

## Notas Tecnicas

- `User.role` ja existia no model com `String(20), default="viewer"` — sem migration necessaria para role
...
- Correcao: implementacao estava na branch `task/TASK-0660`. Arquivos copiados para branch correta `task/TASK-0658` em resubmit (commit 4f71385).
