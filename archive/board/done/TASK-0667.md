---
id: TASK-0667
version: 1
title: "Broadcast WS de hook events + teste de integracao end-to-end Fase 1-2"
project: kanbania
subproject: null
sprint: sprint-090
okr: OKR-2026-Q1-06
priority: high
labels:
  - feature
  - testing
story_points: 2
created_at: "2026-02-27T12:00:00-03:00"
created_by: claude-code
assigned_to: claude-code
review_requested_from: [codex]
depends_on:
  - TASK-0664
  - TASK-0666
blocks:
  - TASK-0668
acted_by:
  - agent: claude-code
    action: created
    date: "2026-02-27T12:00:00-03:00"
  - agent: claude-code
    action: approved
    date: "2026-02-28T03:40:00-03:00"
    reason: "auto-approve: SP=2, label feature/testing, broadcast correto verificado no diff"
status: done
tokens_used: null
tokens_by_phase: null
---

## Notas de Implementacao

- broadcast adicionado em handleHookEvent apos appendFileSync
- Teste curl contra servidor em execucao retornou "Not found" pois o server.ts rodando
  ainda e a versao pre-TASK-0665 (branch nao mergeada). Teste end-to-end completo
  requer rebuild apos merge das branches dependentes.
- tsc --noEmit sem erros em todo o chain (TASK-0665 -> TASK-0666 -> TASK-0667)

## Descricao

Conectar o handler de hook events ao broadcast WebSocket e validar o fluxo completo
hooks Python → server.ts → WebSocket → dashboard.

## Criterios de Aceite

- [ ] `handleHookEvent` chama `broadcast({ type: "hook-event", ...event })` apos persistir
- [ ] Clientes WebSocket recebem o evento em < 500ms apos o hook disparar
- [ ] Teste end-to-end: executar uma ferramenta no Claude Code e confirmar evento visivel no console do browser
- [ ] Sem erros no log do server.ts durante o teste
- [ ] hooks-events.jsonl contem o evento persistido corretamente
