...
acted_by:
  ...
  - agent: "claude-code"
    ...
    reason: "criterios atualizados para getppid() (correto per TASK-0661). session_end.py ja le antes de remover."
status: review
tokens_used: null
...
### session_start.py
- [ ] Gera UUID de sessao e salva em `/tmp/claude-session-{os.getppid()}.id`
- [ ] Chama `send_hook_event("SessionStart", {"ppid": os.getppid()})`
- Nota: usa getppid() — TASK-0661 aprovada define "session_id em /tmp/claude-session-{PPID}.id"
  onde PPID = PID do Claude Code (pai). Criterio original tinha getpid() incorreto.

### session_end.py
- [ ] Le o session_id de `/tmp/claude-session-{PPID}.id`
- [ ] Chama `send_hook_event("SessionEnd", {})` com arquivo ainda presente
- [ ] Remove `/tmp/claude-session-{PPID}.id` apos envio
